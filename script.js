...
let currentMatchIndex = { primary: 0, secondary: 0 }; // Track the current match index
const botPrompts = [
  `Hi there! Welcome to the Interactive Lottery Chatbot Calculator. What would you like to do?<br>
        1: Set up a Lottery Game<br>
        2: Set up a Keno Game<br>
        3: View Calculations from the Games (Excel)<br>
        Type 'exit' to quit.<br><br>
        <a href="instructions.pdf" target="_blank" style="color: #007bff; text-decoration: underline;">
          📄 Download Instructions (PDF)
        </a>`,
  "Does the game have a bonus ball being drawn? Type 'yes' or 'no':",
  "Enter the max number in the matrix (e.g., 69 for 5/69 game):",
  "Enter the number of selected numbers for the matrix:",
  "Enter the secondary matrix size (e.g., 26 for 1/26 game):",
  "Enter the number of selected numbers for the secondary matrix:",
  "Enter the price point of the game:",
  "Enter the number of spots for the Keno game (1-12):"
];
...
function handleUserInput() {
  ...
  if (userInput.toLowerCase() === "exit") {
    appendMessage("bot", "Goodbye! Have a great day! You can close this tab in your browser.");
    return;
  }
  ...
  if (currentStep === 0) {
    if (userInput === "1") {
        ...
        // Option to download the Excel files
        appendMessage(
            "bot",
            ...
            <a href="Keno Games.xlsx" download="Keno Games.xlsx" style="color: #007bff; text-decoration: underline;">
              📊 Download Keno Games (Excel)
            </a>`
        );
    } else {
  ...
  //   }
    ...
    if (userInput.toLowerCase() === "yes") {
        gameData.matrices = 2; // Use 2 matrices if there's a bonus ball
        appendMessage("bot", "Got it! The game uses 2 matrices. " + botPrompts[++currentStep]);
    } else if (userInput.toLowerCase() === "no") {
        gameData.matrices = 1; // Use 1 matrix if there's no bonus ball
        appendMessage("bot", "Got it! The game uses 1 matrix. " + botPrompts[++currentStep]);
    } else {
        appendMessage("bot", "Invalid input. Please type 'yes' or 'no'.");
    }
    ...
    //}
    if (gameData.matrices === 2) {
      appendMessage("bot", botPrompts[++currentStep]); // Proceed to ask for secondary matrix details
  } else {
      appendMessage("bot", botPrompts[6]); // Skip directly to asking for the game price
      currentStep = 6;
  }
    ...
    if (gameData.matrices === 1) {
      ...
      appendMessage(
        "bot",
        `Enter the prize payout for match ${gameData.currentMatchIndex} (or type 'Jackpot'):`
      );
    } else {
      currentPrizeMatrix = [];
      currentMatchIndex = { primary: gameData.selectedNumbers1, secondary: gameData.selectedNumbers2 };
      appendMessage(
        "bot",
        `Enter the prize payout for ${currentMatchIndex.primary} + ${currentMatchIndex.secondary} match (or type 'Jackpot'):`
      );
      currentStep = 7;
    }
  } else if (currentStep === 7) {
    ...
    if (isNaN(spots) || spots < 1 || spots > 12) {
      ...
      gameData.kenoType = spots;
      gameData.prizes = [];
      appendMessage("bot", `Enter the prize payout for ${gameData.kenoType} matches:`);
      currentPrizeMatrix = [];
    ...
    if (userInput.toLowerCase() === "jackpot") {
      ...
      const prize = parseFloat(userInput);
      if (isNaN(prize) || prize < 0) {
        appendMessage("bot", "Invalid input. Please enter a numeric value.");
        return;
      }
      currentPrizeMatrix.push(prize);
    }
    ...
    if (currentPrizeMatrix.length < gameData.kenoType + 1) {
      appendMessage("bot", `Enter the prize payout for ${gameData.kenoType - currentPrizeMatrix.length} matches:`);
    } else {
...
function calculateKenoPayout() {
  ...
  let totalLiability = 0;
  let totalWinners = 0;
  let totalSumWinners = 0; // Final total sum of winners

  // First loop to calculate the final total sum of winners
  for (let matchCount = 0; matchCount <= kenoType; matchCount++) {
    const favorableCombinations =
      binomialCoefficient(20, matchCount) *
      binomialCoefficient(60, kenoType - matchCount);

    totalSumWinners += favorableCombinations; // Add all winners, including $0 prize matches
  }

  // Second loop to calculate odds, liability, etc.
  for (let matchCount = 0; matchCount <= kenoType; matchCount++) {
    const prize = currentPrizeMatrix[kenoType - matchCount]; // Match prizes in reverse order
    const favorableCombinations =
      binomialCoefficient(20, matchCount) *
    ...
    // Ensure favorable combinations are valid
    if (favorableCombinations === 0) continue;

    const expectedWinners = favorableCombinations; // Expected winners for the match
    const odds = totalSumWinners / expectedWinners; // Correct odds calculation
    ...
    if (prize > 0) {
      totalWinners += expectedWinners; // Only count winners for prizes > $0
    }
  }
  ...
  displayResults(results, totalProbability, overallOdds, totalLiability, totalWinners, 0, totalSumWinners);
}



function createDownloadButton(results, totalProbability, overallOdds, totalLiability, totalWinners, grossSales, totalSumWinners) {
  const downloadButton = document.createElement("button");
  downloadButton.innerText = "Download Results (CSV)";
  downloadButton.style.margin = "10px";
  downloadButton.onclick = () => downloadResultsAsCSV(results, totalProbability, overallOdds, totalLiability, totalWinners, grossSales, totalSumWinners);
...
function downloadResultsAsCSV(results, totalProbability, overallOdds, totalLiability, totalWinners, grossSales, totalSumWinners) {
  let csvContent = "data:text/csv;charset=utf-8,";
  ...
  csvContent += `\n"Total Probability","${(totalProbability * 100).toFixed(2)}%"\n`;
  csvContent += `"Overall Odds","1 in ${overallOdds.toFixed(2)}"\n`;
  csvContent += `"Total Liability","$${totalLiability.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}"\n`;
  csvContent += `"Total Number of Winners","${totalWinners.toLocaleString()}"\n`;
  
  if (gameData.type === "Keno"){
    csvContent +=  `"Total Sum of Winners","${totalSumWinners.toLocaleString()}"\n`;
  }
  else{
    csvContent += `"Total Gross Sales","$${grossSales.toLocaleString()}"\n`;
  }
...
// Display results in the chat
function displayResults(results, totalProbability, overallOdds, totalLiability, totalWinners, grossSales, totalSumWinners) {
  appendMessage("bot", "Calculation complete! Here are the results:");
  ...
  //appendMessage("bot", `Total Gross Sales: $${grossSales.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`);
    ...
    // Add Total Sum of Winners for Keno only
    if (gameData.type === "Keno") {
